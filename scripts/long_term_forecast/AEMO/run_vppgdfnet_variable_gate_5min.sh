...
log_dir="${LOG_DIR:-results/aemo_vic1/vppgdfnet_variable_gate_5min_${run_id}}"
...
for pred_len in "${pred_lens[@]}"; do
  log_file="${log_dir}/vppgdfnet_variable_gate_sl${seq_len}_pl${pred_len}.log"
  started_at="$(date -Iseconds)"

  echo "[${started_at}] START VPPGDFNet ablation=variable_gate seq_len=${seq_len} pred_len=${pred_len}"

  set +e
  VPP_ABLATION="variable_gate" \
  PRED_LENS="${pred_len}" \
  SEQ_LEN="${seq_len}" \
  LABEL_LEN="${label_len}" \
  ...
  DES="${DES:-AEMO-5min-vppgdfnet-variable-gate}" \
...

  {
    ...
    csv_escape "variable_gate"; printf ','
  ...
  echo "[${ended_at}] END VPPGDFNet ablation=variable_gate seq_len=${seq_len} pred_len=${pred_len} status=${status} exit_code=${exit_code} mse=${mse} mae=${mae}"
done
