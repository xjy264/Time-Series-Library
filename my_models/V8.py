"""V8：Autoformer 分解 + Crossformer(多尺度交互) + DLinear(趋势)

【拼接模块】
1) series_decomp（Autoformer）
2) Crossformer（跨尺度/分段交互建模，适合 seasonal 多尺度结构）
   - 来源：models/Crossformer.py
3) DLinear（趋势线性外推）

【数据流】
- x_enc -> decomp -> seasonal, trend
- seasonal -> Crossformer -> seasonal_pred
...
from models import Crossformer, DLinear
...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        self.seasonal_model = Crossformer.Model(configs)
